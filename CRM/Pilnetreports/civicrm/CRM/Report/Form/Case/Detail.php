<?php
/*
 * Copied from civicrm/CRM/Form/Report/Case/Detail.php in CiviCRM version 5.65.1
 */
 ...
 */
class CRM_Pilnetreports_civicrm_CRM_Report_Form_Case_Detail extends CRM_Report_Form {
  ...
  public function __construct() {
    ...
    $this->_columns = [
      ...
      'civicrm_contact_provider' => [
        'dao' => 'CRM_Contact_DAO_Contact',
        'fields' => [
          'provider_cid' => [
            'dbAlias' => 'group_concat(DISTINCT contact_provider_civireport.id)',
            'title' => ts('Pro Bono Provider(s)'),
          ],
        ],
      ],
...
";
    ...
    if ($this->isTableSelected('civicrm_contact_provider')) {
      $this->_from .= "
        LEFT JOIN civicrm_relationship provider_relationship
          ON provider_relationship.case_id = {$this->_aliases['civicrm_case']}.id
            AND provider_relationship.is_active
            AND (provider_relationship.end_date IS NULL OR provider_relationship.end_date > now())
  ...
  public function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      if (array_key_exists('civicrm_contact_provider_provider_cid', $row)) {
        $rows[$rowNum]['civicrm_contact_provider_provider_cid'] = $this->_alterProBonoProviderDisplay($rows[$rowNum]['civicrm_contact_provider_provider_cid']);
        $entryFound = TRUE;
      }
   ...
   * @return string semicolon-delimited list of links to the given contacts.
   */
